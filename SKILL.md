...
2. Compose content slides as images.
   ...
   - Generate one best-fit version per content slide by default. Choose the composition automatically based on information density, text amount, and the level of visual detail needed; do not create multiple template/style variants for the user to compare unless explicitly requested.
...
- Do not generate many visual template options by default. Produce a single strongest version per slide, then regenerate only when there is a quality, accuracy, readability, or layout problem.
