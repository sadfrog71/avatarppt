...
- Inserts text-free 16:9 visual canvases generated by GPT Image or MiniMax and
  overlays argument-bearing copy as native editable PowerPoint text.
...
- Requires visual inspection of generated Chinese text only in an explicitly
  approved bitmap-text route; the default editable route forbids text in the
  generated canvas.
...
- Keeps all new slide copy editable by default: titles, body text, KPI values,
  labels, captions, and source notes. Font size has an 18pt hard floor, with
  regular copy at 20pt or larger and content titles normally at 28pt or larger.
...
- Treats anti-AI style as a whole-page composition check rather than a ban on
  graphics: non-text raster or generated-image expression stays within 30%,
  while larger information-bearing frameworks, processes, charts, and evidence
  systems are built with editable native PowerPoint elements.
...
After assembly, run the zero-cost output audit:

```bash
python scripts/audit_pptx.py manifest.example.json --output outputs/pptx-audit.json
```
...
- New plans must declare `image_area_ratio<=0.30` and native
  `slide.editable_text` covering every `exact_text` item.
